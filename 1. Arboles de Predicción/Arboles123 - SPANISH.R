...
# ya que R simplemente ejecutara todo el texto anterior a # en cada línea e ignorará
# lo que se encuentre a la derecha de # (en cada línea, claro). En el editor de texto 
# (editor de script) de mi versión actual de RSTUDIO, el texto que R ignora al ejecutar 
...
# Tratemos de graficar algo más útil para nuestro modesto nivel de estudiantes.
...
# Estos gráficos me siguen dando un poco de dolor de cabeza. Y además salió un
...
# Sí, pero la variable 7 es el Resultado y ~ x1:6. O es dependiente (y) o es
...
# Dividir arbol2 a la mitad. Una mitad será nuestra variable de entrenamiento y la 
...
# ¿Será cierto que "Todas las madres quieren a sus hijos por igual"?
...
# Ok, hasta ahora sabemos que rpart() nos enseñó cosas diferentes sobre dos mitades
...
# Del ejercicio Ya sabemos que con rpart(), diferentes árboles o modelos de predicción
...
# puede predecir correctamente sobre bases de datos diferentes y así, llegar a Roma).

# Sabiendo esto, no necesitamos 3 modelos predictivos. Fabricar fit.arbol3.prune y 
# fit.arbol3.test no es necesarios así que no lo haremos. El leñador entusiasta
# es más que bienvenido a hacerlo por su cuenta, la práctica de copy/paste/edit
...
# Al llamar fit.arbol3.train, utilizamos nuestro árbol de predicción que fabricamos
...
# le pedimos a predict() una predicción de la variable "Resultado" para cada una de
...
# Nada nuevo = Frustrante... así es la vida del analista de datos y del
# pronosticador.
...
# Cuando ejecutamos la función str(), había 10000 líneas, entonces,
# ¿a ver cómo termina nuestra base de predicciones de resultados?
...
# el árbol (modelo predictivo) "fit.arbol3.train").
...
# Guau, la diferencia parece pequeña, ¿de cuánto es?
...
# para 0s o FALSE a nivel agregado (holístico).
...
# ¿Cuál será la precisión atómica, línea por línea, observación especifica predicha
# versus exactamente esa misma observación especifica real? En otras palabras, ¿cuántas
...
# El objetivo detrás de podar el árbol es evitar sobreajuste los datos. 
...
# TRUE. Bien, son iguales! La fuente es de alta calidad y en el futuro volveré a ella
...
# mi escritorio lo más limpio y ordenado posible.
...
# No, identical() está de acuerdo, no hay diferencias. En la realidad, esto sería una
...
# en el problema de sobreajuste (overfitting) de un árboles de decisión. ¿Cuándo ocurre
# este sobreajuste? Sucede cuando un árbol de decisión (un modelo predictivo)
...
# vimos como rpart() ignoro algunos atributos (el 3 y 4) al generar el árbol predictivo
...
# cada línea u observación, cada árbol fabricado (entrenado) generar su 
...
# Las soluciones son variadas. Se puede eliminar la(s) observación(es) con
...
# Una forma de resumir la cantidad de tipeo en R, es usar un sinónimo para "TODOS LOS
# ATRIBUTOS". Y ese sinónimo es el punto ".". Quedaría entonces más corto, de la siguiente forma
...
# y nunca será posible reproducir exactamente los mismo resultados.
...
# dentro de él), se podrá utilizar la función predict() como venimos haciendolo.
...
# respectivamente en el gráfico de Gini de la derecha, podrían perfectamente tener un orden
# opuesto en el gráfico de la izquierda.
...
# No obstante, el segundo gráfico no dice algo muy útil:

# ntrees = 1000 en randomForest() fue un buen argumento ya que la varianza de error
# se aplana después árbol número 550 aproximadamente. Genial!!
...
# Comparación correcta, claro, la matemática funciona. 
# ¿Es una comaparación justa, ¡¡Realmente NO!!

# A nivel atómico, observación por observación, ¿quién es mejor?

# Sabíamos que rpart() tenía una exactitud, comparando observación por observación
# a nivel atómico (i.e. línea 35667 real vs. 35667 predicha), del 98.06%. 
# Recordemos:
...
# Lo cual significa que randomForest() es el más certero a nivel atómico.
