...
# sección veremos brevemente  la creación de una serie de tiempo, su descomposición estacional,
# modelado con modelos exponencial y ARIMA, y pronósticos con el paquete forecast.
...
# paquete zoo que se cargó en nuestra sesión de R cuando cargamos el paquete forecast al
# comienzo.

# La clase zoo fue diseñada para manejar datos de series de tiempo ordenadas por 
# índices de tiempo arbitrarios. 
# Decimos arbitrario porque un índice de ordenamiento por tiempo puede ser regular, donde el 
# espacio entre observaciones es el mismo (observaciones diarias, semanales, etc.), o las 
# observaciones pueden tener separaciones de irregulares (1 día entre observaciones 1 y 2,
# tres días entre observaciones 2 y 3, seis días entre la observaciones 3 y 4, etc.).

# Esencialmente, un objeto zoo atribuye información de fecha almacenado en un vector con la
# a la información de datos de cada observación almacenados en una matriz.

# Para crear un objeto zoológico se necesita datos ordenados por un índice de tiempo.
# Obviamente, el índice de tiempo debe tener el mismo número de filas como el objeto de datos
# y este índice de tiempo puede ser cualquier vector que contiene observaciones ordenadas. 
# Típicamente, el índice de tiempo es un objeto de la clase "Date", "POSIXct", "yearmon",
# "yearqtr" o "TimeDate".

# Ya que tenemos los datos ordenados por año, mes y día, lo primero que necesitamos hacer 
# entonces es unir las 3 variables (o columnas) "Año", "Mes" y "Día", en 
...
?ISOdate
?with
fechas <- with(gripes, ISOdate(Año, Mes, Día))
...
# POSIXct, que es uno de los formato que requiere zoo. Pero tenemos, para cada día, el dato de
# horario ("12:00:00") que es el mismo para todas las observaciones y como no estaban 
# originalmente, ahora tampoco nos sirven, no agregan ningún valor así que podemos eliminar los
# horarios convirtiendo del formato "POSIXct" a formato "Fecha", el cual también es aceptado 
# por zoo.
...
# Primero confirmamos los formatos necesarios para cálculos de tiempo estén presentes
str(gripes.z)

# Y luego que los datos estén ordenados de igual manera que nuestra base de datos (data.frame)
# original
...
# Belleza! A primera vista vemos que el ciclo estacional aparenta ser de 1 año, con las
# cantidades de casos de gripe aumentando durante el invierno en norte américa.
...
# Esta descomposición (separación) de se logra mediante la función stl(). 
...
# Tenemos la función de los HoltWinters() en la instalación base de R y se puede utilizar 
...
# Excelente, nos da intervalos de confianza. Si miramos el resumen del pronóstico3, podremos
# ver a qué intervalos se refiere cada color y explicarlo al presentar el grafico.
...
# negativas con
# gripe, claramente no necesitamos las predicciones negativas, así que podemos modificar
# un poco el grafico
